    ...
    render(){
        ...
        const comp = this.state.complaints.map(cmpln => (
            <div className={styles.ComplaintA}>
                ...
                <div className={styles.ComplaintAFlex}>
                <div>
                    <input onKeyDown={(event) => this.props.sendReplyHandler(cmpln.id,event)} />
                </div>
                <div>
                <button onClick={ (event) => this.props.markClosedHandler(cmpln.id,event)}> Mark as Closed </button>
                </div>
                </div>
