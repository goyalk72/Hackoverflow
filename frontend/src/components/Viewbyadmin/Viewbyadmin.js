...
class Viewbyadmin extends Component {
    ...
    sendReplyHandler = (id,event) => {
        ...
        if (event.keyCode === 13){
            ...
            axios.post(URL+'/update',reply)
            .then( () => function updatestate(){
                var today = new Date();
                ...
                for (let i =0; i< complaints.length; i++){
                    if (complaints[i].id == id){
                        complaints[i].replies.push({date: today, msg: event.target.value});
                        break;
                    }
                }
    ...
    searchHandler = (event) => {
        if (event.keyCode === 13){
            ...
            }).then((res) => function updatestate(res){
    ...
    render() {
        ...
        return (
            <div>
                ...
                <ComplaintA complaints={this.state.complaints} sendReplyHandler={this.sendReplyHandler} markClosedHandler={this.markClosedHandler}/>
            </div>);
    }
}
